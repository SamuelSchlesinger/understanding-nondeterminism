...
| Broering-Lokam SAT 2003, published 2004, publisher abstract. [broering-width][broering-width] | SAT/Circuit-SAT with width parameters: polynomial-time backtrack search for logarithmic cutwidth/pathwidth, quasipolynomial-time backtrack search for logarithmic treewidth/branchwidth, and polynomial-time regular-resolution refutations for logarithmic treewidth. | Publisher abstract and primary preview pp.162-163 verified. The preview explicitly removes fanout dependence. The remaining full text was not retrieved. | Prior width-based Circuit-SAT algorithms. Its abstract does not specify the candidate excess parameter or the numerical gate exponent. |
...
| --- | --- | --- | --- |
| Nurk 2009, institutional preprint abstract. [nurk-sat][nurk-sat] | General binary Boolean circuit with `s` internal gates: Circuit-SAT in `O(2^{0.4058s})` time. | Branching algorithm according to the abstract and subsequent primary descriptions. Full original text not retrieved; no explicit space theorem checked. | A relevant historical gate-count benchmark. The proved uniform consequence has a smaller gate exponent with exponential space; exact historical priority is unresolved. |
...
records the remaining retrieval gaps.
...
Morizumi, Cavalar-Oliveira, and (in the focused follow-up) Lialina.
Wang et al., Nurk, Broering-Lokam, and the original graph-search papers have the expressly noted full-text gaps.
