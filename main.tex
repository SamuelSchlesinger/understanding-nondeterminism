...
\usepackage{bookmark}
\hypersetup{pdftitle={Understanding Nondeterminism: From Witnesses and Structure to Faster SAT},
...
\vspace{-1.5em}
\begin{abstract}
Nondeterminism asks whether one globally consistent witness makes a
deterministic check accept. This deep dive follows that idea from
computation paths and verifier relations to shared circuit values,
exact counting tables, and structural bounds on eliminating witnesses.
...
Pinning input tables also retains the counting exponent for search,
lexicographic rank and unrank, and expected-time exact uniform sampling.
...
The repository includes a Rust implementation of the full automatic
counting route, a source-to-proof resource argument, and finite checks.
Structured example timings are separate from the asymptotic guarantee;
the methods appendix records their scope and the AI-assisted workflow.
\end{abstract}
...
\input{sections/counting-walkthrough}
\input{sections/algorithm-overview}
...
Sections~\ref{sec:foundations}--\ref{sec:algorithm-overview}
build the route from computation paths to shared witnesses, evaluate a
complete counting-table example, and state the restricted SAT guarantee.
The next two sections establish the elementary circuit-size bounds and
explain why witness counts alone miss reusable computation.
Section~\ref{sec:consistency-graph} improves the size exponent by combining
a circuit-specific consistency budget with established graph-width tools,
and proves the uniform counting algorithm in
Section~\ref{sec:counting-algorithm}.
Section~\ref{sec:algorithm-consequences} then derives polynomial-time
counting at low cycle rank, sharper input-sensitive exponents, search,
lexicographic indexing, and exact uniform generation.
...
\input{sections/algorithm-consequences}

\section{What the structural bounds do and do not force}
\input{sections/critical-balance}
...
The shared-witness counterexample in Section~\ref{sec:foundations}
shows why existence cannot be pushed independently through an AND gate.
...
The Exponential Time Hypothesis (ETH) and Strong Exponential Time
Hypothesis (SETH) are hypotheses about algorithms. A lower bound
